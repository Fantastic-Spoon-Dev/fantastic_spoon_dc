...
export default class HeadlinesCommand extends Command {
  async run(ctx: CommandContext<typeof options>) {
    ...
    const base_url =
      "https://newsapi.org/v2/top-headlines?pageSize=10&apiKey=" +
      process.env.NEWS_API_KEY +
      "&category=" +
      (ctx.options.category || "general");
    ...
    for (let index = 0; index < resJson.articles.length; index++) {
      ...
      try {
        ...
        if (fl_res && fl_res.ok) {
          ...
          if (fl_resJson?.url) shortUrl = fl_resJson.url;
        } else {
          logger.error("Failed to shorten URL");
        }
      } catch (e) {
        logger.error("Failed to parse shortener response JSON");
      }
      menu.addOption(
      ...
      cardsHtml += `
				<div class="flex justify-center w-full">
					<div class="w-[645px] bg-[${dark[2]}] shadow-lg rounded-2xl p-4">
						<h2 class="text-lg font-semibold mb-2 break-words text-[${dark[1]}]">${
        index + 1
      }. ${article.title}</h2>
    ...
    const imgBuffer = await imgGen(html, 650, 200);
